...
import meshcat
import mujoco
from robots.robot_env import ROOT_JOINT_INDEX
...
def add_noise_to_state(rmodel, x, scale = .01, noise_levels=[1, np.pi/2, np.pi/2, 1, 1]):
...
def load_arrays(filename='data_compressed'):
    ...
    full_path = os.path.join(script_dir, 'trajectory/trajectory_data/', filename + '.npz')
