...
if(skip_this_file)
  foreach(component ${cito_FIND_COMPONENTS})
    if(NOT "cito_${component}_FOUND")
      set(skip_this_file FALSE)
    endif()
  endforeach()
endif()
...
set("cito_INCLUDE_DIRS" "${PACKAGE_PREFIX_DIR}/include")
set("CITO_INCLUDE_DIRS" "${PACKAGE_PREFIX_DIR}/include")
set("cito_DOXYGENDOCDIR" "${PACKAGE_PREFIX_DIR}/share/doc/cito/doxygen-html")
set("CITO_DOXYGENDOCDIR" "${PACKAGE_PREFIX_DIR}/share/doc/cito/doxygen-html")
set("cito_DEPENDENCIES" "pinocchio;crocoddyl;eigenpy")
set("cito_PKG_CONFIG_DEPENDENCIES" "example-robot-data;ipopt")
...
set("_cito_PKG_CONFIG_LIBDIR" "${pcfiledir}/../../lib")
set("_cito_PKG_CONFIG_LIBS_LIST" "")
if(_cito_PKG_CONFIG_LIBS_LIST)
  string(FIND ${_cito_PKG_CONFIG_LIBS_LIST} ", " _is_comma_space)
  while(_is_comma_space GREATER -1)
    string(REPLACE ", " "," _cito_PKG_CONFIG_LIBS_LIST "${_cito_PKG_CONFIG_LIBS_LIST}")
    string(FIND ${_cito_PKG_CONFIG_LIBS_LIST} ", " _is_comma_space)
  endwhile()
  string(REPLACE " " ";" _cito_PKG_CONFIG_LIBS_LIST "${_cito_PKG_CONFIG_LIBS_LIST}")
  set(LIBDIR_HINTS ${_cito_PKG_CONFIG_LIBDIR})
  foreach(component ${_cito_PKG_CONFIG_LIBS_LIST})
  ...
  endforeach()
endif(_cito_PKG_CONFIG_LIBS_LIST)

set("cito_LIBRARIES" ${_PACKAGE_CONFIG_LIBRARIES})
set("CITO_LIBRARIES" ${_PACKAGE_CONFIG_LIBRARIES})
...
IF(COMMAND ADD_REQUIRED_DEPENDENCY)
  FOREACH(pkgcfg_dep ${cito_PKG_CONFIG_DEPENDENCIES})
...
include("${CMAKE_CURRENT_LIST_DIR}/citoTargets.cmake")
...
foreach(component ${cito_FIND_COMPONENTS})
  ...
  if(EXISTS ${comp_file})
    ...
    set(cito_${component}_FOUND FALSE)
  endif()
  if(cito_${component}_FOUND)
    message(STATUS "cito: ${component} found.")
  else()
    if(cito_FIND_REQUIRED_${component})
      message(FATAL_ERROR "cito: ${component} not found.")
    else()
      message(STATUS "cito: ${component} not found.")
    endif()
  endif()
endforeach()
check_required_components("cito")
